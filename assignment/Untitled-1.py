...
def create_assignment(request, subject_id):
    ...
    classes_taught = teacher_profile.classes.all() # get the classes from the teacher profile

    if request.method == 'POST':
        form = AssignmentCreateForm(request.POST, request.FILES, teacher_profile=teacher_profile)
        if form.is_valid():
            for classroom in classes_taught: #we iterate over the classroom to create one instance for each classroom
...
# File: c:\Users\tjman\OneDrive\school\assignment\views.py
...
def submitted_assignments(request, classroom_id):
    ...
    assignments = Assignment.objects.filter(classroom=classroom, teacher=teacher_profile)
